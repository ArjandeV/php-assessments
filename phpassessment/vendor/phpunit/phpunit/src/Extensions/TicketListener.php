...
/**
 * Base class for fileOwnersTest listeners that interact with an issue tracker.
 */
    ...
    public function endTest(PHPUnit_Framework_Test $test, $time)
    {
        if (!$test instanceof PHPUnit_Framework_WarningTestCase) {
            if ($test->getStatus() == PHPUnit_Runner_BaseTestRunner::STATUS_PASSED) {
                ...
                $newStatus  = 'closed';
                $message    = 'Automatically closed by PHPUnit (fileOwnersTest passed).';
                $resolution = 'fixed';
                $cumulative = true;
            } elseif ($test->getStatus() == PHPUnit_Runner_BaseTestRunner::STATUS_FAILURE) {
                $ifStatus   = ['closed'];
                $newStatus  = 'reopened';
                $message    = 'Automatically reopened by PHPUnit (fileOwnersTest failed).';
                $resolution = '';
                $cumulative = false;
            } else {
                return;
            ...
            foreach ($tickets as $ticket) {
                // Remove this fileOwnersTest from the totals (if it passed).
                if ($test->getStatus() == PHPUnit_Runner_BaseTestRunner::STATUS_PASSED) {
                    unset($this->ticketCounts[$ticket][$name]);
                }

                // Only close tickets if ALL referenced cases pass
                // but reopen tickets if a single fileOwnersTest fails.
                if ($cumulative) {
                    // Determine number of to-pass tests:
                    if (count($this->ticketCounts[$ticket]) > 0) {
                        // There exist remaining fileOwnersTest cases with this reference.
                        $adjustTicket = false;
                    } else {
